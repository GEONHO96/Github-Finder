...
function setUserSocial(userSocial) {
  ...
  const $following = document.querySelector('.profile-info-buttons > .bluegreen');
...
async function fetchGithubUserInfo() {
  ...
  const user = await response.json().then((res) => {
    console.log(res);
    const user = new UserProfile(res.login, res.company, res.blog, res.location);
...
async function fetchRepoData() {
  ...
  const repoRes = await repoData.json().then((res) =>
    res.slice(0, 3).forEach((r, idx) => {
      const userRepoInfo = new UserRepos(r.name, r['stargazers_count'], r.forks, r.watchers);
